server '198.199.119.91', port: 22, roles: [:web, :app, :db], primary: true
...
set :ssh_options,     { forward_agent: true, user: fetch(:user), keys: %w(~/.ssh/blackbook_rsa) }
...
namespace :envvars do
  ...
  task :load do
    ...
    on roles(:app) do
      ...
      value = capture("cat /home/blackbook/master.key")
...
namespace :deploy do
  ...
  task :check_revision do
    on roles(:app) do
      ...
      # Update this to your branch name: master, master, etc. Here it's master
